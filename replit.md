...
- **SID Pipeline (`sid_main.py`)**: Identifies speakers using pre-enrolled embeddings. **Preprocessing settings are automatically loaded from PKL metadata** - no separate `--preprocess` flag needed. When multiple PKLs are provided via `--embedding`, each runs with its own embedded preprocessing settings.
  - **Score Normalization** (`--score-norm`): Optional z-norm score normalization. Z-norm standardizes scores per speaker based on impostor statistics. This handles the "some speakers are easier" problem where different speakers have different score distributions. Requires z-norm stats to be added via `add_znorm.py` first.
- **Z-Norm Utility (`add_znorm.py`)**: Standalone script to add z-norm statistics to existing PKL files. Updates PKL files in place by computing per-speaker impostor statistics (mean, std) from the enrolled centroids. Usage: `python add_znorm.py model.pkl` - no re-enrollment needed.
  - **Cohort Scoring** (`--cohort-size`): Number of top impostors (hard negatives) to use. Instead of using all impostors, computes μ/σ from only the N most similar speakers. This is the standard practice that produces larger/more stable sigma values and focuses on confusing cases. Recommended: 50-100 for datasets with 100+ speakers. Example: `python add_znorm.py model.pkl --cohort-size 50`
  - **Sigma Floor** (`--sigma-floor`): Minimum reliable sigma value (default 0.03). Speakers with sigma below this threshold are marked "unreliable" and will use raw cosine scores at inference time instead of z-normalized scores. This prevents tiny sigma values from causing score explosions that turn speakers into "sinks".
  - **Reliability Flag**: Each speaker's z-norm stats include a `reliable` boolean. At inference, unreliable speakers fall back to raw cosine similarity automatically.
