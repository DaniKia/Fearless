...
-   **Flexible Track Selection**: Supports processing various tracks (e.g., ASR\_track2, SID, SD\_track2) from the Fearless Steps Challenge dataset using a `--track` parameter.
-   **Data Handling**: Manages loading audio, transcripts, and speaker labels from a structured Google Drive dataset, with automatic detection of dataset types (ASR\_track2 vs. SID) and corresponding label formats.
