...
def display_batch_summary_extended(results, show_confusion_matrix=False):
    """
    ...
    for k in [3, 5]:
        if k in top_k_acc:
            print(f"Top-{k} Accuracy       : {top_k_acc[k]:.2f}%")
    print("="*80)
    ...
    print("-"*80)
    print(f"{'Speaker':20s} {'Predicted':>10s} {'Correct':>10s} {'Precision':>12s}")
    print("-"*80)
    for speaker_id in sorted(precision_stats.keys()):
        stats = precision_stats[speaker_id]
        print(f"{speaker_id:20s} {stats['predicted']:10d} {stats['correct']:10d} {stats['precision']:11.1f}%")
    print("="*80)
