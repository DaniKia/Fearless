...
def load_all_asr_track2_speaker_labels(label_dir, dataset='Dev'):
    """
    Load all ASR_track2 speaker labels from the uttID2spkID file.
    
    Args:
        label_dir: Directory containing ASR_track2 label files
        dataset: Dataset name (Dev, Train, or Eval)
        
    Returns:
        Dictionary mapping utterance_id to speaker_id
    """
    cache_key = f"asr_track2_{label_dir}_{dataset}"
    
    if cache_key in _sid_label_cache:
        return _sid_label_cache[cache_key]
    
    label_file = config.find_asr_track2_speaker_label_file(label_dir=label_dir, dataset=dataset)

    labels = {}

    if not os.path.exists(label_file):
        print(f"Warning: ASR_track2 speaker label file not found: {label_file}")
        return labels
    ...
    try:
        ...
        _sid_label_cache[cache_key] = labels
        print(f"Loaded {len(labels)} ASR_track2 speaker labels from {dataset} dataset")
        
    except Exception as e:
        print(f"Error loading ASR_track2 speaker labels: {e}")
    
    return labels

def load_speaker_label_auto(audio_path, audio_filename, dataset='Dev'):
    """
    Automatically detect dataset type and load the appropriate speaker label.
    
    Args:
        audio_path: Path to audio directory (used for auto-detection)
        audio_filename: Name of the audio file
        dataset: Dataset name (Dev, Train, or Eval)
    ...
    Returns:
        Speaker ID string, or None if not found
    """
    dataset_type, detected_split = config.detect_dataset_info_from_path(audio_path)
    
    # Use detected split if provided, otherwise use the passed dataset parameter
    if detected_split:
        dataset = detected_split
    ...
    if dataset_type == 'SID':
        ...
        label_dir = config.get_asr_track2_speaker_label_path(dataset)
        labels = load_all_asr_track2_speaker_labels(label_dir, dataset)
